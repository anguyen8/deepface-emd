...
class FaceDataset(data.Dataset):
    def __init__(self, data_list_file, data_dir, phase='train', input_shape=(1, 128, 128)):
        self.phase = phase
    ...
    def __getitem__(self, index):
        ...
        splits = sample.split(',')
        img_path = os.path.join(self.data_dir, splits[0])
        data = Image.open(img_path).resize((self.input_shape[1], self.input_shape[2]))
        data = data.convert('L')
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        splits = sample.split(',')
        img_path = splits[0]
        # print('img_path: {} - label: {}'.format(img_path, splits[1]))
        data = Image.open(img_path).resize((self.input_shape[1], self.input_shape[2]))
        # data = Image.open(img_path).resize(self.input_shape)
        data = data.convert('L')
        data = self.transforms(data)
        label = np.int32(splits[1])
        # print('label: ', label)
