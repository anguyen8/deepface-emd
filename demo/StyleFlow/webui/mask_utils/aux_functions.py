...
import random
from mask_utils.create_mask import texture_the_mask, color_the_mask
...
def mask_face(image, face_location, six_points, angle, args=None, type="surgical"):
    debug = False
...
def detect_lmks(image_path, predictor, size=(128, 128)):
    image = cv2.imread(image_path)
    image = cv2.resize(image, size)
    shape = predictor(image, dlib.rectangle(0,0,size[0],size[1]))
    shape = face_utils.shape_to_np(shape)
    face_landmarks = shape_to_landmarks(shape)
    return face_landmarks

def mask_face_alignment(image_path, args):
    # Read the image
    ...
    face_landmarks = args.predictor.get_landmarks(input)
        # shape = face_utils.shape_to_np(shape)
    # face_landmarks = shape_to_landmarks(shape)
...
def mask_image(image, detector, predictor, mask_type='random'):
    # Read the image
    # input = io.imread(image_path)
    # image = cv2.imread(image_path)
    original_image = image.copy()
    ...
    if mask_type == "random":
        available_mask_types = get_available_mask_types()
        # print('available_mask_types: ', available_mask_types)
        mask_type = random.choice(available_mask_types)
    ...
    for (i, face_location) in enumerate(face_locations):
        ...
        if mask_type != "all":
            if len(masked_images) > 0:
                image = masked_images.pop(0)
            image, mask_binary = mask_face(
                image, face_location, six_points_on_face, angle, None, type=mask_type
            )
